...
module.exports = (app) => {
	...
	app.get("/render/didyoumean", async (req, resp) => {
		...
		if (top.length > 40) return utils.error(resp, "Параметр 'top' не может содержать в себе больше 40 символов!", usage, examples);
		if (bottom.length > 40) return utils.error(resp, "Параметр 'bottom' не может содержать в себе больше 40 символов!", usage, examples);

		const canvas = createCanvas(1302, 316);
		const ctx = canvas.getContext("2d");
		const bg = await loadImage(path.join(__dirname, "../..", "./assets/images/didyoumean.png"));
