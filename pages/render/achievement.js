...
const icons = readdirSync(
  path.join(__dirname, "../..", "./assets/images/achievement")
).filter((file) => Number(file.slice(0, -4)));

module.exports = (app) => {
  const examples = [
    "/render/achievement?text=Minecraft!",
    `/render/achievement?text=Minecraft!&icon=${random(
      1,
      icons.length
  ...
  app.get("/render/achievement", async (req, resp) => {
    ...
    if (!text)
      return utils.error(resp, "Пропущен параметр 'text'", usage, examples);
    if (text.length > 300)
      return utils.error(
        resp,
        "Параметр 'text' не может содержать в себе больше 300 символов!",
        usage,
        examples
      );
    if (title.length > 100)
      return utils.error(
        resp,
        "Параметр 'title' не может содержать в себе больше 100 символов!",
        usage,
        examples
      );
    if (isNaN(icon) || icon < 1 || icon > icons.length)
      return utils.error(
        resp,
        `Параметр 'image' должен быть числом от 1 до ${icons.length}!`,
        usage,
        examples
      );
    ...
    registerFont(
      path.join(__dirname, "../..", "./assets/fonts/Minecraft.ttf"),
      { family: "Minecraft" }
    );
    ...
    const front = await loadImage(
      path.join(__dirname, "../..", `./assets/images/achievement/${icon}.png`)
    );
    ctx.drawImage(front, 0, 0);

    const end = await loadImage(
      path.join(__dirname, "../..", "./assets/images/achievement/achend.png")
    );
    ctx.drawImage(end, x - 20, 0);

    const mid = await loadImage(
      path.join(__dirname, "../..", "./assets/images/achievement/achmid.png")
    );
