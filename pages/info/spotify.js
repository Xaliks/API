module.exports = (app) => {
  ...
  const usage = "/info/spotify?type=String(artist|track|album|playlist|user)&query=String";
  ...
  app.get("/info/spotify", async (req, resp) => {
    ...
    if (type === "artist") {
      const albums = [await fetch(`https://api.spotify.com/v1/artists/${data.id}/albums`).then((resp) => resp.items), []];
      ...
      const top10tracks = [
        await fetch(`https://api.spotify.com/v1/artists/${data.id}/top-tracks?market=US`).then((resp) => resp.tracks),
        [],
      ];
    ...
    if (type === "album") {
      ...
      let tracks = [await fetch(`https://api.spotify.com/v1/albums/${data.id}/tracks`).then((resp) => resp.items), []];
    ...
    if (type === "playlist") {
      ...
      return resp.send({
        ...
        images: data.images,
        total_tracks_duration: tracks.reduce((a, b) => a.duration_ms + b.duration_ms),
...
async function get(type, id) {
  const resp = await fetch(`https://api.spotify.com/v1/${type + "s"}/${encodeURIComponent(getIDbyURI(id))}`);
  ...
  function getIDbyURI(URI) {
    if (URI.startsWith("https://open.spotify.com/")) return URI.split("/")[4].split("?")[0];
