...
module.exports = (app) => {
  ...
  app.get("/render/captcha", async (req, resp) => {
    ...
    if (text.length > 350)
      return utils.error(
        resp,
        "Параметр 'text' не может содержать в себе больше 350 символов!",
        usage,
        examples
      );
    ...
    const front = await loadImage(
      path.join(__dirname, "../..", "./assets/images/captcha/start.png")
    );
    ctx.drawImage(front, 0, 0);

    const end = await loadImage(
      path.join(__dirname, "../..", "./assets/images/captcha/end.png")
    );
    ctx.drawImage(end, x - 201, 0);

    const mid = await loadImage(
      path.join(__dirname, "../..", "./assets/images/captcha/mid.png")
    );
