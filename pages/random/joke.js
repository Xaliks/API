...
module.exports = {
  async run() {
    ...
    const data_ru = await fetch("https://anekdotov.me/random/").then((resp) =>
      resp.text()
    );
    ...
    const category_ru = $_ru('span[class="bord"] a').toArray()[0].children[0]
      .data;
    const joke_ru = $_ru('textarea[type="text"]')
      .toArray()[0]
      .children[0].data.replace(/<br \/>+/g, "\n"); // это можно сделать как-то проще, но я не знаю как

    const category_en = $_en('h3[class="text-muted"]')
      .text()
      .split("category ")[1];
    let joke_en = $_en('div[class="text-center"] p')
      .text()
      .replace(" Powered by jokes.one Jokes API.", "");
    if (joke_en === "")
      joke_en = $_en('div[class="text-left head-room"] p')
        .text()
        .replace(" Powered by jokes.one Jokes API.", "");
